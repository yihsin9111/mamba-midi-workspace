...
class Pop1k7_MIDI_Dataset(object):
    '''
    Pop1k7 dataset, in midi format, with no condition.
    '''
    def __init__(self, root_path, is_incontext = False, codec_layer = 4, L = 2588) -> None:
        '''
        root_path = /home/yihsin/datasets/pop1k7_cne
        '''
        self.root = root_path
        ...
        self.codec_layer = codec_layer
        self.length = L
        self.special_token_id = 364 # total 363 kinds of token
        self.is_incontext = is_incontext
    
    def __getitem__(self, idx):
        path = os.path.join(self.root, self.pieces[idx])
        data = read_pkl(path)
        # print(f"data shape: {data.shape}")
        # data = data[:self.codec_layer, :]
        data = np.array([data])
        ...
        #return torch.LongTensor(data[:, :-1]), mask[0], torch.LongTensor(data[:, 1:]), description
        return torch.LongTensor(data[:, :-1]), mask[0], torch.LongTensor(data[:, 1:]), "description" # description will 
...
if __name__ == '__main__':
    ...
    train_data = Pop1k7_MIDI_Dataset(root_path = "/home/yihsin/dataset/pop1k7_cne", codec_layer=1, is_incontext = False)
