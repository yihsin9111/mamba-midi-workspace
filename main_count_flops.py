...
def main():
    ...
    input_shape = (batch_size, 1, 2588)
    # import random
    # print(torch.randint(3, 5, (batch_size, 1, 2588)))
    k = {
        'x' : torch.randint(1024, (batch_size, 1, 2588)),
        'condition' : torch.randint(3, 50, (batch_size, 60, 768)).to(torch.float),
        'condition_mask': torch.ones(batch_size, 60),
    }
    
    # print(k['x'].dtype(), type(k['condition']), type(k['condition_mask']))
    
    flops, macs, params = calculate_flops(  model=model.music_model, 
                                            # kwargs = , 
